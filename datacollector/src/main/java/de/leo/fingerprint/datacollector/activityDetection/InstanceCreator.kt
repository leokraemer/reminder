...
class FingerPrintAttributes(var numberOfDataPoints : Int = 1) : ArrayList<Attribute>() {
    ...
    init {
        ...
        for (i in 0 until numberOfDataPoints) {
            ...
            //detected activity
            add(Attribute(DETECTED_ACTIVITY + i, ACTIVITIES))
            addAttribute(DETECTED_ACTIVITY_CONFIDENCE + i)
            add(Attribute(DETECTED_ACTIVITY_2 + i, ACTIVITIES))
            addAttribute(DETECTED_ACTIVITY_2_CONFIDENCE + i)
            add(Attribute(DETECTED_ACTIVITY_3 + i, ACTIVITIES))
...
fun createFingerprint(jitai: Jitai,
    ...
    val fingerprint = Fingerprint("fingerprint", fingerPrintAttrs)
...
internal fun deSerializeWifi(wifiNames: String): List<WifiInfo> {
    ...
    return wifis.map {
        try {
            val values = it.split(";")
            WifiInfo(values[0], values[1].toInt(), values[2], values[3], values[4].toInt())
        } catch (e: Exception) {
...
private fun createInstance(context: Context,
    ...
    val sensorDataSetsInFrame: List<SensorDataSet?> =
        getSensorDatasetsByTime(windowDefiningSensorData, db, fingerPrintAttrs)
            ...
            if (dataPoint != null) {
                ...
                with(fingerprint) {
                    with(fingerPrintAttrs) {
                        ...
                        values[getAttributeIndex(PEAK_LIGHT_VALUE + i)] = normalizePower(lightPeak,
                                                                                         5000.0)
                        values[getAttributeIndex(MIN_LIGHT_VALUE + i)] = normalizePower(lightMin,
                                                                                        500.0)
                        values[getAttributeIndex(MEAN_LIGHT_VALUE + i)] = normalizePower(lightMean,
                                                                                         5000.0)
                        ...
                        val wifis = deSerializeWifi(dataPoint.wifiInformation!!)
                        ...
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_1 + i)] = getWifiLevel(wifi)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_2 + i)] = getWifiLevel(wifi2)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_3 + i)] = getWifiLevel(wifi3)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_4 + i)] = getWifiLevel(wifi4)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_5 + i)] = getWifiLevel(wifi5)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_6 + i)] = getWifiLevel(wifi6)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_7 + i)] = getWifiLevel(wifi7)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_8 + i)] = getWifiLevel(wifi8)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_9 + i)] = getWifiLevel(wifi9)
                        values[getAttributeIndex(WIFI_SIGNAL_STRENGTH_10 + i)] = getWifiLevel(wifi10)
                        if (connected != null)
