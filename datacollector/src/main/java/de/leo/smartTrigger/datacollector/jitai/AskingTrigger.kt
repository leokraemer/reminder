...
class AskingTrigger(val jitaiId: Int, val validity: Long) : Trigger {
    ...
    override fun check(context: Context, sensorData: SensorDataSet): Boolean {
        if (preconditions.all { check(context, sensorData) }) {
            ...
            val latestYes = events.lastOrNull { it.eventType == NaturalTriggerJitai.NOTIFICATION_TRIGGER_YES }
            val latestNo = events.lastOrNull { it.eventType == NaturalTriggerJitai.NOTIFICATION_TRIGGER_NO }
