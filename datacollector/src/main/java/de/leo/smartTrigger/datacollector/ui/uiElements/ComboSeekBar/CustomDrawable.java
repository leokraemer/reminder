package de.leo.smartTrigger.datacollector.ui.uiElements.ComboSeekBar;
...
public class CustomDrawable extends Drawable {
    ...
    public CustomDrawable(Drawable base, ComboSeekBar slider, float thumbRadius,
                          List<ComboSeekBar.Dot> dots, int color, int textSize, boolean
                                  isMultiline, float padding) {
        ...
        setBounds(0, 0, getBounds().width(), (int) (mThumbRadius + mTextMargin + mTextHeight *
                textLines + mPadding * 2));
    }

    private float toPix(int size) {
        return TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, size, mySlider.getContext()
    ...
    public final void draw(Canvas canvas) {
        ...
        //draw line
        canvas.drawLine(mDots.get(0).mX, yLine, mDots.get(mDots.size() - 1).mX, yLine,
                selectLinePaint);
        for (ComboSeekBar.Dot dot : mDots) {
          /*  if (dot.isSelected) {
                canvas.drawLine(mDots.get(0).mX, yLine, dot.mX, yLine, selectLinePaint);
                canvas.drawLine(dot.mX, yLine, mDots.get(mDots.size() - 1).mX, yLine,
                unselectLinePaint);
            }*/
...
/*		if (dot.isSelected) {
    ...
    public final int getIntrinsicHeight() {
        if (mIsMultiline) {
            return (int) (selectLinePaint.getStrokeWidth() + mDotRadius + (mTextHeight *
                    textLines) * 2 + mTextMargin + mPadding * 2);
        } else {
