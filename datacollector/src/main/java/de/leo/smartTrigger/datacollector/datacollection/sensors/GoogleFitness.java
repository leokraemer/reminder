...
/**
 * Created by Yunlong on 8/7/2017.
 */

public class GoogleFitness {

    private static final int GOOGLE_FIT_PERMISSIONS_REQUEST_CODE = 5;
    private static final String TAG = "GoogleFitness";
    private static final boolean isStepCounter = true;
    private GoogleApiClient mClient;
    private Context context;
    private GoogleFitnessListener googleFitnessListener;

    public GoogleFitness(Context context) {
        this.context = context;
        googleFitnessListener = (GoogleFitnessListener) context;
                        ...
                        new GoogleApiClient.ConnectionCallbacks() {
                            ...
                            public void onConnectionSuspended(int i) {
                                ...
                                if (i == GoogleApiClient.ConnectionCallbacks.CAUSE_NETWORK_LOST) {
                                    Log.i(TAG, "Connection to google fit lost.  Cause: Network " +
                                            "Lost.");
                                } else if (i == GoogleApiClient.ConnectionCallbacks
                                        .CAUSE_SERVICE_DISCONNECTED) {
                                    Log.i(TAG, "Connection lost. google fit lost. Reason: Service" +
                                            " Disconnected");
                                }
                            }
                        }
                )
                /*    .enableAutoManage((AppCompatActivity)context, 2, new GoogleApiClient
    ...
    public void disconnect() {
        //mClient.stopAutoManage((AppCompatActivity)context);
                ...
                public void onResult(@NonNull DailyTotalResult totalResult) {
                    if (totalResult.getStatus().isSuccess()) {
                        ...
                        long total = totalSet.isEmpty()
                                ? 0
                                : totalSet.getDataPoints().get(0).getValue(Field.FIELD_STEPS)
                                .asInt();
                        googleFitnessListener.onReceivedStepsCounter(total);
