 ...
 */
class MyActivity(private val context: Context) : GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, ResultCallback<Status> {
    ...
    var confidentActivity: DetectedActivity = DetectedActivity(DetectedActivity.UNKNOWN, 0);
    ...
    fun registerReceiver() {
        LocalBroadcastManager.getInstance(context).registerReceiver(mBroadcastReceiver, IntentFilter(Constants.STRING_ACTION))
    }
    ...
    fun requestActivityUpdates() {
        if (!mGoogleApiClient.isConnected) {
            ...
            ActivityRecognition.ActivityRecognitionApi.requestActivityUpdates(mGoogleApiClient, 5000, activityDetectionPendingIntent).setResultCallback(this)
        }
    }
    ...
    fun removeActivityUpdates() {
        if (!mGoogleApiClient.isConnected) {
            ...
            ActivityRecognition.ActivityRecognitionApi.removeActivityUpdates(mGoogleApiClient, activityDetectionPendingIntent).setResultCallback(this)
        }
    }
    ...
    inner class ActivityDetectionBroadcastReceiver : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (detectedActivities[0].confidence > 20) {
                confidentActivity = detectedActivities[0]
            }
