...
import com.example.leo.datacollector.GeofencesWithPlayServices.Constants
...
class JitaiDatabase private constructor(val context: Context) : SQLiteOpenHelper(context,
    ...
    companion object {
        @Volatile private var INSTANCE: JitaiDatabase? = null
    ...
    fun getSensorDataset(events: List<JitaiEvent>): List<Pair<SensorDataSet, String>> {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                val s = SensorDataSet(timestamp, user, recording, id, DetectedActivity
                (DetectedActivity.UNKNOWN, 0),
                                      totalStepsToday,
                                      ambientSound, location, gps, wifiName, bluetoothDevices,
                                      screenState, weather)
                val event = events[events.indexOfFirst { it.sensorDatasetId == id }]
    ...
    fun get3DSensorValues(rec_id: Int, table: String): MutableList<Pair<Long, FloatArray>> {
        val c = readableDatabase.query(table, arrayOf(X, Y, Z, TIMESTAMP), "$RECORDING_ID = ?",
    ...
    fun getALL3DSensorValuesNoTimestamp(start: Long, end: Long, table: String): MutableList<DoubleArray> {
        val c = readableDatabase.query(table,
                                       arrayOf(X, Y, Z),
                                       "$TIMESTAMP > ? AND " + "$TIMESTAMP < ?",
                                       arrayOf(start.toString(), end.toString()),
    ...
    fun getSensorValues(rec_id: Int, table: String): MutableList<Pair<Long, Double>> {
        val c = readableDatabase.query(table, arrayOf(X, TIMESTAMP), "$RECORDING_ID = ?",
                                       arrayOf(rec_id.toString()), null, null, null, null)
    ...
    fun getRecording(rec_id: Int): ActivityRecord {
        ...
        c.close()
        record.accelerometerData = get3DSensorValues(rec_id, TABLE_REAL_TIME_ACC)
        record.magnetData = get3DSensorValues(rec_id, TABLE_REAL_TIME_MAG)
        record.gyroscopData = get3DSensorValues(rec_id, TABLE_REAL_TIME_GYRO)
        record.proximity = getSensorValues(rec_id, TABLE_REALTIME_PROXIMITY)
        record.orientationData = getOrientationSensorValues(rec_id)
        record.ambientLight = getSensorValues(rec_id, TABLE_REALTIME_LIGHT)
        record.pressure = getSensorValues(rec_id, TABLE_REALTIME_AIR)
        return record
    }

    private fun getOrientationSensorValues(rec_id: Int): MutableList<Pair<Long, FloatArray>> {
        val c = readableDatabase.query(TABLE_REAL_TIME_ROT, arrayOf(X, Y, Z, SCALAR, TIMESTAMP),
                                       "$RECORDING_ID = ?",
                                       arrayOf(rec_id.toString()), null, null, null, null)
        val list = mutableListOf<Pair<Long, FloatArray>>()
        if (c.count > 0) {
    ...
    fun enterJitai(jitai: Jitai): Jitai {
        ...
        val id = writableDatabase.insert(TABLE_JITAI, null, cv)
        jitai.id = id.toInt()
        return jitai
    }
    ...
    fun getActiveJitai(): MutableList<Jitai> {
        val c = readableDatabase.query(TABLE_JITAI, null, "$JITAI_ACTIVE = ? and $JITAI_DELETED < 1",
    ...
    fun deleteJitai(id: Int) {
        ...
        writableDatabase.update(TABLE_JITAI, cv, "$ID = ?", arrayOf(id.toString()))
    }
    ...
    fun enterJitaiEvent(id: Int, timestamp: Long, eventName: Int, sensorDatasetId: Long) {
        ...
        writableDatabase.insert(TABLE_JITAI_EVENTS, null, cv)
    }
    ...
    fun getJitaiEventsForTraining(id: Int): MutableList<JitaiEvent> {
        val c = readableDatabase.query(TABLE_JITAI_EVENTS, null, "$JITAI_ID = ? AND " +
                "$JITAI_EVENT IN ('${Jitai.NOTIFICATION_SUCCESS}' ,'${Jitai.NOW}' , '${Jitai
                        .NOTIFICATION_FAIL}'," + "'${Jitai.NOTIFICATION_DELETED}')",
                                       arrayOf(id.toString()),
                                       null, null, null)
        val list = mutableListOf<JitaiEvent>()
        if (c.moveToFirst()) {
...
val REALTIME_TABLES = arrayOf(TABLE_REALTIME_LIGHT,
                              TABLE_REALTIME_PROXIMITY, TABLE_REALTIME_AIR,
                              TABLE_REALTIME_HUMIDITY, TABLE_REALTIME_TEMPERATURE)
